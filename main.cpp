...
namespace yf {
    ...
    //std::filesystem::path ypath = std::filesystem::current_path().parent_path().parent_path().parent_path() / "config.yaml";
    YAML::Node config = YAML::LoadFile("config.yaml");
    
    const int firstShape = config["first_shape"]["shape"].as<int>();
    const int secondShape = config["second_shape"]["shape"].as<int>();

    const float fs_size = config["first_shape"]["size"].as<float>();
    const float ss_size = config["second_shape"]["size"].as<float>();

    const int fs_R_color = config["first_shape"]["RGBA_color"][0].as<int>();
    const int fs_G_color = config["first_shape"]["RGBA_color"][1].as<int>();
    const int fs_B_color = config["first_shape"]["RGBA_color"][2].as<int>();
    const float fs_T_color = config["first_shape"]["RGBA_color"][3].as<float>();
    sf::Color fs_color(fs_R_color, fs_G_color, fs_B_color, fs_T_color);

    const int ss_R_color = config["second_shape"]["RGBA_color"][0].as<int>();
    const int ss_G_color = config["second_shape"]["RGBA_color"][1].as<int>();
    const int ss_B_color = config["second_shape"]["RGBA_color"][2].as<int>();
    const float ss_T_color = config["second_shape"]["RGBA_color"][3].as<float>();
    sf::Color ss_color(ss_R_color, ss_G_color, ss_B_color, ss_T_color);

    const int fs_R_out_color = config["first_shape"]["RGBA_outline_color"][0].as<int>();
    const int fs_G_out_color = config["first_shape"]["RGBA_outline_color"][1].as<int>();
    const int fs_B_out_color = config["first_shape"]["RGBA_outline_color"][2].as<int>();
    const float fs_T_out_color = config["first_shape"]["RGBA_outline_color"][3].as<float>();
    sf::Color fs_out_color(fs_R_out_color, fs_G_out_color, fs_B_out_color, fs_T_out_color);

    const int ss_R_out_color = config["second_shape"]["RGBA_outline_color"][0].as<int>();
    const int ss_G_out_color = config["second_shape"]["RGBA_outline_color"][1].as<int>();
    const int ss_B_out_color = config["second_shape"]["RGBA_outline_color"][2].as<int>();
    const float ss_T_out_color = config["second_shape"]["RGBA_outline_color"][3].as<float>();
    sf::Color ss_out_color(ss_R_out_color, ss_G_out_color, ss_B_out_color, ss_T_out_color);
}
...
struct App {
    ...
    void updateTextures() {
        ...
        firstIm.drawPoints(yf::firstShape, yf::fs_size, yf::fs_color, yf::fs_out_color);
        secondIm.drawPoints(yf::secondShape, yf::ss_size, yf::ss_color, yf::ss_out_color);
        thirdIm.drawPoints(yf::secondShape, yf::ss_size, yf::ss_color, yf::ss_out_color);
        ...
        if (ImGui::BeginMainMenuBar()) {
            ...
            if (ImGui::BeginMenu("File")) {
                ...
                if (ImGui::MenuItem("Import 1st Image Points", nullptr, false, true)) {
                    firstIm.importPoints(yf::firstShape, yf::fs_size, yf::fs_color, yf::fs_out_color);
                }
                if (ImGui::MenuItem("Import 2nd Image Points", nullptr, false, true)) {
                    secondIm.importPoints(yf::secondShape, yf::ss_size, yf::ss_color, yf::ss_out_color);
                };
                ImGui::Separator();
