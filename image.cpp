...
void Image::drawPoints(int shapeType, float shapeSize, sf::Color shapeColor, sf::Color shapeOutlineColor) {
    pointsTexture.clear(sf::Color(0, 0, 0, 0));
    if(shapeType == 1)
    {
        for (std::size_t i = 0; i < ast.countPts(); ++i) {
            sf::CircleShape tempCircle(shapeSize / 2);
            cv::Point point(ast.getPosition(i));
            tempCircle.setPosition(sprite.getTransform().transformPoint(point.x - shapeSize / 2, point.y - shapeSize / 2));
            tempCircle.setFillColor(shapeColor);
            if (hoveredCircleIndex == i) {
                tempCircle.setOutlineThickness(3);
                tempCircle.setOutlineColor(shapeOutlineColor);
            }
    ...
    {
        for (std::size_t i = 0; i < ast.countPts(); ++i) {
            sf::RectangleShape tempRectangle(sf::Vector2f(shapeSize, shapeSize));
            cv::Point point(ast.getPosition(i));
            tempRectangle.setPosition(sprite.getTransform().transformPoint(point.x - pointRadius / 2, point.y - pointRadius / 2));
            tempRectangle.setFillColor(shapeColor);
            if (hoveredCircleIndex == i) {
                tempRectangle.setOutlineThickness(3);
                tempRectangle.setOutlineColor(shapeOutlineColor);
            }
            pointsTexture.draw(tempRectangle);
        }
    }
    else if(shapeType == 3)
    {
        for (std::size_t i = 0; i < ast.countPts(); ++i) {
            CrossShape tempCross(sf::Vector2f(shapeSize, shapeSize));
            cv::Point point(ast.getPosition(i));
            tempCross.setPosition(sprite.getTransform().transformPoint(point.x - shapeSize / 2, point.y - shapeSize / 2));
            tempCross.setFillColor(shapeColor);
            if (hoveredCircleIndex == i) {
                tempCross.setOutlineThickness(3);
                tempCross.setOutlineColor(shapeOutlineColor);
            }
            pointsTexture.draw(tempCross);
        }
    }
}
...
void Image::importPoints(int shapeType, float shapeSize, sf::Color shapeColor, sf::Color shapeOutlineColor) {
    nfdchar_t *outPath = nullptr;
    nfdresult_t result = NFD_OpenDialog("txt", nullptr, &outPath);
    if (result == NFD_OKAY) {
        ast.loadPts(outPath);
        drawPoints(shapeType, shapeSize, shapeColor, shapeOutlineColor);
    } else {
        std::cout << "nfd error" << std::endl;
    }
}
